...
// #include "ArduinoLog.h" // Uncomment if you want to use ArduinoLog explicitly
...
// ADC Objects (using ADC_ATTEN_DB_11 for 0-3.3V range)
//adcObj damperRightRear(ADC2_CHANNEL_8, ADC_ATTEN_DB_11);
//adcObj damperLeftRear(ADC1_CHANNEL_5, ADC_ATTEN_DB_11);
//adcObj BSPD(ADC1_CHANNEL_6, ADC_ATTEN_DB_11);
//adcObj brakePressure(ADC1_CHANNEL_7, ADC_ATTEN_DB_11);
...
void setup()
{
 ...
 {
   Serial.begin(115200);
   // Log.begin(LOG_LEVEL_ERROR, &Serial); // Uncomment and define LOG_LEVEL if using ArduinoLog
   Serial.println("\n------------------------------------");
 ...
 //damperLeftRear.setCalibrationPoints(pot_cal_dataL, 5);
 //damperRightRear.setCalibrationPoints(pot_cal_dataR, 5);
...
void loop()
{
 ...
 prepareCANMessage();
 transmitCANMessage();
    delay(180); // for eaven distribution of can messages
...
bool initializeCAN()
{
 ...
 else
 {
   ...
   twai_driver_uninstall(); // Uninstall if start fails
   return false;
 }
 return true;
}
...
void readSensorData()
{
 // Damper readings (using getVoltage_mV for meaningful values)
 //damperRearLeftVoltage = damperLeftRear.getMappedValue_LUT();
 //damperRearRightVoltage = damperRightRear.getMappedValue_LUT();
 ...
 //brakePressureVoltage = brakePressure.getVoltage_mV();
